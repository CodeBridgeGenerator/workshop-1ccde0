  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((res) => {
                set_entity(res || {});
                const cusrtomerName = Array.isArray(res.cusrtomerName)
            ? res.cusrtomerName.map((elem) => ({ _id: elem._id, name: elem.name }))
            : res.cusrtomerName
                ? [{ _id: res.cusrtomerName._id, name: res.cusrtomerName.name }]
                : [];
        setCusrtomerName(cusrtomerName);
            })
            .catch((error) => {
                console.log({ error });
  ...
  const copyPageLink = () => {
    ...
    return (
        ...
        <div className="col-12 flex flex-column align-items-center">
            ...
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Plate No</label><p className="m-0 ml-3" >{_entity?.plateNo}</p></div>
<div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Year</label><p className="m-0 ml-3" >{Number(_entity?.year)}</p></div>
            <div className="col-12 md:col-6 lg:col-3"><label className="text-sm text-gray-600">Cusrtomer Name</label>
                    {cusrtomerName.map((elem) => (
